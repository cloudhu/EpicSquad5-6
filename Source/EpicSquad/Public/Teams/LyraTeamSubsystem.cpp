...
bool ULyraTeamSubsystem::ChangeTeamForActor(AActor* ActorToChange, int32 NewTeamIndex)
{
	...
	if (APlayerState_Base* LyraPS = const_cast<APlayerState_Base*>(FindPlayerStateFromActor(ActorToChange)))
	{
		LyraPS->GetEpicSquadPC()->SetGenericTeamId(NewTeamID);
		return true;
	}
...
int32 ULyraTeamSubsystem::FindTeamFromObject(const UObject* TestObject) const
{
	...
	if (const AActor* TestActor = Cast<const AActor>(TestObject))
	{
		...
		if (const APlayerState_Base* LyraPS = FindPlayerStateFromActor(TestActor))
		{
			return LyraPS->GetEpicSquadPC()->GetGenericTeamId();
		}
	}
...
const APlayerState_Base* ULyraTeamSubsystem::FindPlayerStateFromActor(const AActor* PossibleTeamActor) const
{
	if (PossibleTeamActor != nullptr)
	{
		if (const APawn* Pawn = Cast<const APawn>(PossibleTeamActor))
		{
			...
			if (const APlayerState_Base* LyraPS = Pawn->GetPlayerState<APlayerState_Base>())
			{
				return LyraPS;
			}
		}
		...
		{
			if (const APlayerState_Base* LyraPS = Cast<APlayerState_Base>(PC->PlayerState))
			{
				return LyraPS;
			}
		}
		else if (const APlayerState_Base* LyraPS = Cast<const APlayerState_Base>(PossibleTeamActor))
		{
			return LyraPS; 
		}
