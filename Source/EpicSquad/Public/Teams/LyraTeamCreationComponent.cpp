...
void ULyraTeamCreationComponent::ServerAssignPlayersToTeams()
{
	...
	{
		if (APlayerState_Base* LyraPS = Cast<APlayerState_Base>(PS))
		{
...
void ULyraTeamCreationComponent::ServerChooseTeamForPlayer(APlayerState_Base* PS)
{
	if (PS->IsOnlyASpectator())
	{
		if (AEpicSquadPlayerController* PlayerController=Cast<AEpicSquadPlayerController>(PS->GetPlayerController()))
		{
			PlayerController->SetGenericTeamId(FGenericTeamId::NoTeam);
		}
	}
	else
	{
		AEpicSquadPlayerController* PlayerController=Cast<AEpicSquadPlayerController>(PS->GetPlayerController());
		const FGenericTeamId TeamID = IntegerToGenericTeamId(GetLeastPopulatedTeamID());
		PlayerController->SetGenericTeamId(TeamID);
	}
}
...
void ULyraTeamCreationComponent::OnPlayerInitialized(AGameModeBase* GameMode, AController* NewPlayer)
{
	...
	if (APlayerState_Base* LyraPS = Cast<APlayerState_Base>(NewPlayer->PlayerState))
	{
		ServerChooseTeamForPlayer(LyraPS);
	}
}
...
int32 ULyraTeamCreationComponent::GetLeastPopulatedTeamID() const
{
	...
	if (NumTeams > 0)
	{
		...
		for (APlayerState* PS : GameState->PlayerArray)
		{
			if (APlayerState_Base* LyraPS = Cast<APlayerState_Base>(PS))
			{
				AEpicSquadPlayerController* PlayerController=LyraPS->GetEpicSquadPC();
				const int32 PlayerTeamID = PlayerController->GetGenericTeamId();
