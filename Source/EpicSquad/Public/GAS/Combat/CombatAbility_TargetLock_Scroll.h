...
#include "CombatAbility_TargetLock_Scroll.generated.h"
...
/**
 * Target Pick Policy: decide which target should be the right one player want.
 */
UENUM(BlueprintType)
...
/**
* Locks on a target, acquired by a certain targeting preset. Enable Scroll Target: add support for switching locked targets using the mouse wheel.
*/
UCLASS()
class EPICSQUAD_API UCombatAbility_TargetLock_Scroll : public UNinjaCombatGameplayAbility
{
	GENERATED_BODY()

public:
	UCombatAbility_TargetLock_Scroll();
...
protected:
	...
	/** Defines if we need to find new target when current tracked target is out of range. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Combat Ability|Target Lock", meta = (EditCondition = "bTrackDistance"))
	bool bFindNewTargetOnDistanceExceeded;

	/** Optional Gameplay Effect applied if a target is acquired. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Combat Ability|Target Lock")
	TSubclassOf<UGameplayEffect> TargetLockEffectClass;

	/** Optional Gameplay Effect applied if to targets locked by this avatar. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Combat Ability|Target Lock")
	TSubclassOf<UGameplayEffect> SourceLockEffectClass;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Ninja Combat|Abilities")
	void ApplyTargetLockEffect(AActor* Target);
...
private:
	...
	ETargetPickPolicy TargetPickPolicy = ETargetPickPolicy::NearestOne;

	/** Handle for the applied Targeting Handle (source targeting an enemy). */
	FActiveGameplayEffectHandle ActiveTargetLockEffectHandle;

	/** Handle for the applied Targeting Handle (enemy targeted by the source). */
	TArray<FActiveGameplayEffectHandle> ActiveSourceLockEffectHandles;
