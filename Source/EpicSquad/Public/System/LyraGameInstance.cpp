...
#include "UI/FrontendGameplayTags.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(LyraGameInstance)

namespace Lyra
{
	static bool GBTestEncryption = false;
...
#if UE_WITH_DTLS
	static bool GBUseDtlsEncryption = false;
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& InArgs, UWorld* InWorld)
			{
				if (InArgs.Num() == 1)
				{
					...
					const FTimespan CertExpire = FTimespan::FromDays(365);
					if (TSharedPtr<FDTLSCertificate> Cert = FDTLSCertStore::Get().CreateCert(CertExpire, CertName); Cert.IsValid())
					{
						if (const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / FPaths::MakeValidFileName(FString::Printf(TEXT("%s.pem"), *CertName)); !Cert->ExportCertificate(CertPath))
						{
...
void ULyraGameInstance::Init()
{
	...
	if (UGameFrameworkComponentManager* ComponentManager = GetSubsystem<UGameFrameworkComponentManager>(this); ensure(ComponentManager))
	{
...
AEpicSquadPlayerController* ULyraGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AEpicSquadPlayerController>(Super::GetPrimaryPlayerController(false));
}
...
void ULyraGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, const bool bSuccess, const FText Error, const ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
...
void ULyraGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Lyra::GBUseDtlsEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Lyra::GBTestDtlsFingerprint)
			{
				// Generate server cert for this identifier, post the fingerprint
				const FTimespan CertExpire = FTimespan::FromHours(4);
...
void ULyraGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	...
	if (Lyra::GBUseDtlsEncryption)
	{
		...
		if (const APlayerController* const PlayerController = GetFirstLocalPlayerController(); PlayerController && PlayerController->PlayerState && PlayerController->PlayerState->GetUniqueId().IsValid())
		{
...
void ULyraGameInstance::OnPreClientTravelToSession(FString& URL) const
{
	// Add debug encryption token if desired.
	if (Lyra::GBTestEncryption)
	{
#if UE_WITH_DTLS
		if (Lyra::GBUseDtlsEncryption)
		{
			if (const APlayerController* PlayerController = GetFirstLocalPlayerController(); PlayerController && PlayerController->PlayerState && PlayerController->PlayerState->GetUniqueId().IsValid())
			{
				const FUniqueNetIdRepl& PlayerUniqueId = PlayerController->PlayerState->GetUniqueId();
				const FString EncryptionToken = PlayerUniqueId.ToString();
