...
{
	// Set up our lazy init function for classes that are not currently loaded.
...
void ULyraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AEpicSquadPlayerController* PC = Cast<AEpicSquadPlayerController>(CurViewer.InViewer))
		{
			...
			if (AEpicSquadPlayerCharacter* Pawn = Cast<AEpicSquadPlayerCharacter>(PC->GetPawn()))
			{
			...
			if (AEpicSquadPlayerCharacter* ViewTargetPawn = Cast<AEpicSquadPlayerCharacter>(CurViewer.ViewTarget))
			{
